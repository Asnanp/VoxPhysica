...
class VocalMorphV2(nn.Module):
    ...
    def __init__(
        ...
        self.mixup_probability = float(self.hyperparameters.mixup_probability)
        ...
        if not 0.0 <= self.mixup_probability <= 1.0:
            raise ValueError(
                f"mixup_probability must be in [0, 1], got {self.mixup_probability}"
            )
        ...
        self.height_bin_head = nn.Sequential(
            nn.LayerNorm(self.hyperparameters.fused_dim),
            nn.Linear(
                self.hyperparameters.fused_dim,
                self.hyperparameters.regression_hidden_dim // 2,
            ),
            nn.GELU(),
            nn.Dropout(self.hyperparameters.branch_dropout),
            nn.Linear(self.hyperparameters.regression_hidden_dim // 2, 3),
        )
        ...
        self.loss_module = VocalTractSimulatorLossV2(
            ...
            robust_huber_weight=self.hyperparameters.robust_huber_weight,
            focal_after_epoch=self.focal_after_epoch,
            focal_ema_decay=self.hyperparameters.focal_ema_decay,
            focal_gamma=self.hyperparameters.focal_gamma,
    ...
    def _normalized_height_bin_centers(
        self, device: torch.device, dtype: torch.dtype
    ) -> torch.Tensor:
        centers = torch.tensor(
            [
                float(self.speaker_alignment_config.height_bin_prior_short_cm),
                float(self.speaker_alignment_config.height_bin_prior_medium_cm),
                float(self.speaker_alignment_config.height_bin_prior_tall_cm),
            ],
            device=device,
            dtype=dtype,
        )
        stats = self.target_stats.get("height", {}) if self.target_stats else {}
    ...
    def _apply_feature_mixup(
        ...
        if self.mixup_probability < 1.0:
            probability_gate = torch.rand(
                batch_size, device=device, dtype=torch.float32
            ) < self.mixup_probability
            applied = applied & probability_gate
    ...
    def _forward_once(
        ...
        height_bin_probs = torch.softmax(height_bin_logits, dim=-1)
        height_bin_centers = self._normalized_height_bin_centers(
            device=device, dtype=height_features.dtype
        )
        height_bin_prior_mu = (height_bin_probs * height_bin_centers.view(1, -1)).sum(
            dim=-1
        )
        height_bin_blend = max(
            0.0, min(0.95, float(self.speaker_alignment_config.height_bin_prior_blend))
        )
        height_mu_regression = height_out["mu"]
        height_mu = (
            (1.0 - height_bin_blend) * height_mu_regression
            + height_bin_blend * height_bin_prior_mu
        )
        ...
        output: Dict[str, Any] = {
            ...
            "height_bin_probs": height_bin_probs,
            "height_bin_prior_mu": height_bin_prior_mu,
            "height_mu_regression": height_mu_regression,
...
def build_vocalmorph_v2(config: dict) -> VocalMorphV2:
    ...
    hyperparameter_cfg = dict(v2_cfg)
    aug_cfg = config.get("training", {}).get("augmentation", {})
    if "mixup_alpha" not in hyperparameter_cfg and "mixup_alpha" in aug_cfg:
        hyperparameter_cfg["mixup_alpha"] = aug_cfg["mixup_alpha"]
    if "mixup_probability" not in hyperparameter_cfg and "mixup_p" in aug_cfg:
        hyperparameter_cfg["mixup_probability"] = aug_cfg["mixup_p"]
    hyperparameters = dataclass_from_mapping(ModelHyperparameters, hyperparameter_cfg)
