...
def aggregate_by_speaker(
    ...
    for speaker_id, idxs in grouped.items():
        ...
        for key, value in preds.items():
            ...
            elif resolved_method == "attention_weighted":
                # Attention-based pooling using quality and uncertainty
                temperature = 1.0  # Temperature for softmax
                if vals.ndim == 1:
                    # Compute attention weights from quality and inverse uncertainty
                    if var is not None:
                        uncertainty_weights = 1.0 / (var + eps)
                        attention_scores = combined_subset * uncertainty_weights
                    else:
                        attention_scores = combined_subset
                    
                    # Apply softmax to get attention weights
                    attention_weights = torch.softmax(attention_scores / temperature, dim=0)
                    mean = (vals * attention_weights).sum()
                    
                    # Compute variance with attention weights
                    if len(idxs) > 1:
                        weighted_diff = vals - mean
                        agg_var = (attention_weights * weighted_diff.pow(2)).sum()
                    else:
                        agg_var = torch.zeros((), device=device)
                else:
                    # Multi-dimensional case
                    if var is not None:
                        uncertainty_weights = 1.0 / (var + eps)
                        attention_scores = combined_subset.unsqueeze(-1) * uncertainty_weights
                    else:
                        attention_scores = combined_subset.unsqueeze(-1)
                    
                    # Apply softmax per dimension
                    attention_weights = torch.softmax(attention_scores / temperature, dim=0)
                    mean = (vals * attention_weights).sum(dim=0)
                    
                    if len(idxs) > 1:
                        weighted_diff = vals - mean.unsqueeze(0)
                        agg_var = (attention_weights * weighted_diff.pow(2)).sum(dim=0)
                    else:
                        agg_var = torch.zeros_like(mean)
