...
class VocalTractSimulatorLossV2(nn.Module):
    ...
    def _height_loss(
        ...
        if current_epoch >= self.focal_after_epoch:
            ...
            difficulty = (residual.detach() / denom).clamp(0.25, 3.0)
            focal_weights = difficulty.pow(max(self.focal_gamma, 0.0))
            focal_weights = focal_weights / focal_weights.mean().clamp(min=1e-6)
            per_sample = per_sample * focal_weights
        ...
        if height_raw is not None and current_epoch >= int(
            ...
            male_short_w = float(
                self.speaker_alignment.height_bin_loss_weight_male_short
            )
            female_short_w = float(
                self.speaker_alignment.height_bin_loss_weight_female_short
            )
            male_tall_w = float(
                self.speaker_alignment.height_bin_loss_weight_male_tall
            )
            female_tall_w = float(
                self.speaker_alignment.height_bin_loss_weight_female_tall
            )
            extreme_short_w = float(
                self.speaker_alignment.height_extreme_loss_weight_short
            )
            extreme_tall_w = float(
                self.speaker_alignment.height_extreme_loss_weight_tall
            )
            needs_weighting = max(
                abs(short_w - 1.0),
                abs(medium_w - 1.0),
                abs(tall_w - 1.0),
                abs(male_short_w - 1.0),
                abs(female_short_w - 1.0),
                abs(male_tall_w - 1.0),
                abs(female_tall_w - 1.0),
                abs(extreme_short_w - 1.0),
                abs(extreme_tall_w - 1.0),
            ) > 1e-6
            if needs_weighting:
                height_raw = height_raw.to(device=device, dtype=torch.float32)[valid]
                sample_weights = torch.full_like(per_sample, medium_w)
                short_mask = height_raw < 160.0
                tall_mask = height_raw >= 175.0
                sample_weights = torch.where(
                    short_mask, sample_weights * short_w, sample_weights
                )
                sample_weights = torch.where(
                    tall_mask, sample_weights * tall_w, sample_weights
                )
                gender = targets.get("gender")
                if gender is not None:
                    gender = gender.to(device=device, dtype=torch.long)[valid]
                    male = gender == 1
                    female = gender == 0
                    sample_weights = torch.where(
                        short_mask & male,
                        sample_weights * male_short_w,
                        sample_weights,
                    )
                    sample_weights = torch.where(
                        short_mask & female,
                        sample_weights * female_short_w,
                        sample_weights,
                    )
                    sample_weights = torch.where(
                        tall_mask & male,
                        sample_weights * male_tall_w,
                        sample_weights,
                    )
                    sample_weights = torch.where(
                        tall_mask & female,
                        sample_weights * female_tall_w,
                        sample_weights,
                    )
                sample_weights = torch.where(
                    height_raw <= float(self.speaker_alignment.height_extreme_short_cm),
                    sample_weights * extreme_short_w,
                    sample_weights,
                )
                sample_weights = torch.where(
                    height_raw >= float(self.speaker_alignment.height_extreme_tall_cm),
                    sample_weights * extreme_tall_w,
                    sample_weights,
                )
    ...
    def _height_distribution_loss(
        self,
        preds: Mapping[str, torch.Tensor],
        targets: Mapping[str, torch.Tensor],
        device: torch.device,
    ) -> torch.Tensor:
        """Discourage collapsed height ranges inside mixed-height batches."""
        current_epoch = int(
            targets.get("epoch", preds.get("current_epoch", self.current_epoch))
        )
        if current_epoch < int(self.speaker_alignment.height_distribution_start_epoch):
            return torch.zeros((), device=device)

        target = targets.get("height")
        mu = preds.get("height_mu")
        if target is None or mu is None:
            return torch.zeros((), device=device)

        target = target.to(device=device, dtype=torch.float32)
        mu = mu.to(device=device, dtype=torch.float32)
        valid = torch.isfinite(target) & torch.isfinite(mu)
        min_items = max(2, int(self.speaker_alignment.height_distribution_min_items))
        if int(valid.sum().item()) < min_items:
            return torch.zeros((), device=device)

        target = target[valid]
        mu = mu[valid]
        target_std = target.std(unbiased=False)
        pred_std = mu.std(unbiased=False)
        std_loss = F.smooth_l1_loss(pred_std, target_std)

        target_range = target.max() - target.min()
        pred_range = mu.max() - mu.min()
        range_loss = F.smooth_l1_loss(pred_range, target_range)

        tail_loss = torch.zeros((), device=device)
        if mu.numel() >= 6:
            k = max(1, int(round(float(mu.numel()) * 0.20)))
            k = min(k, mu.numel() // 2)
            pred_low = torch.topk(mu, k=k, largest=False).values.mean()
            target_low = torch.topk(target, k=k, largest=False).values.mean()
            pred_high = torch.topk(mu, k=k, largest=True).values.mean()
            target_high = torch.topk(target, k=k, largest=True).values.mean()
            tail_loss = F.smooth_l1_loss(pred_low, target_low) + F.smooth_l1_loss(
                pred_high, target_high
            )

        return (
            float(self.speaker_alignment.height_distribution_std_weight) * std_loss
            + float(self.speaker_alignment.height_distribution_range_weight)
            * range_loss
            + float(self.speaker_alignment.height_distribution_tail_weight) * tail_loss
        )
    ...
    def _height_bin_loss(
        ...
        labels = torch.ones_like(height_raw, dtype=torch.long)
        labels = torch.where(height_raw < 160.0, torch.zeros_like(labels), labels)
        labels = torch.where(height_raw >= 175.0, torch.full_like(labels, 2), labels)
        class_weights = torch.tensor(
            [
                float(self.speaker_alignment.height_bin_loss_weight_short),
                float(self.speaker_alignment.height_bin_loss_weight_medium),
                float(self.speaker_alignment.height_bin_loss_weight_tall),
            ],
            device=device,
            dtype=torch.float32,
        )
    ...
    def _formant_height_consistency_loss(
        self,
        preds: Mapping[str, torch.Tensor],
        targets: Mapping[str, torch.Tensor],
        device: torch.device,
    ) -> torch.Tensor:
        """Additional loss to ensure formant frequencies are consistent with predicted height."""
        if not self.toggles.use_physics_branch:
            return torch.zeros((), device=device)

        formant_spacing = preds.get("formant_spacing_pred")
        ...
        # Expected formant spacing from predicted height: Δf = c / (2 * VTL) = c / (2 * height/ratio)
        expected_spacing = self.constants.speed_of_sound_cm_per_s / (2.0 * (pred_height_raw / self.vtl_height_ratio))

        valid = torch.isfinite(formant_spacing) & torch.isfinite(expected_spacing) & (formant_spacing > 0)
        if valid.sum() == 0:
            return torch.zeros((), device=device)

        # Consistency loss between predicted formant spacing and height-derived spacing
        consistency_loss = F.smooth_l1_loss(
            formant_spacing[valid],
            expected_spacing[valid],
            reduction="mean"
        )
    ...
    def forward(
        ...
        formant_height_loss = self._formant_height_consistency_loss(preds, targets, device)
        ...
        height_distribution_loss = self._height_distribution_loss(
            preds, targets, device
        )
        height_bin_loss = self._height_bin_loss(preds, targets, device)
        ...
        losses = {
            ...
            "gender": gender_loss,
            "vtsl": vtsl_loss,
            "formant_height_consistency": formant_height_loss,
            "physics_penalty": physics_penalty,
            "diversity": diversity_loss,
            "height_ranking": ranking_loss,
            "height_bin": height_bin_loss,
            ...
            "height_distribution": height_distribution_loss,
        ...
        total = (
            ...
            + self.loss_weights.formant_height_consistency
            * losses["formant_height_consistency"]
            ...
            + self.loss_weights.height_bin * losses["height_bin"]
            ...
            * losses["speaker_height_ranking"]
            + float(self.speaker_alignment.height_distribution_weight_max)
            * losses["height_distribution"]
        )
