...
class FeatureAugmentConfig:
    """Ultra-aggressive on-the-fly feature-level augmentation parameters for 15-epoch 3.6cm MAE target."""

    # Gaussian noise (ultra-aggressive for rapid regularization)
    noise_p: float = 0.75
    noise_std: float = 0.05

    # Time masking (SpecAugment-style - ultra-aggressive)
    time_mask_p: float = 0.65
    time_mask_max_frac: float = 0.20  # max 20% of sequence length

    # Feature dimension masking (ultra-aggressive)
    feat_mask_p: float = 0.55
    feat_mask_max_frac: float = 0.15  # max 15% of feature dims

    # Feature scaling (per-utterance gain - ultra-aggressive)
    scale_p: float = 0.55
    scale_std: float = 0.15

    # Temporal jitter (random frame dropout - ultra-aggressive)
    temporal_jitter_p: float = 0.45
    temporal_jitter_max_frac: float = 0.12  # max 12% of frames

    # Frequency masking (SpecAugment-style - ultra-aggressive)
    freq_mask_p: float = 0.50
    freq_mask_max_frac: float = 0.25  # max 25% of feature dimensions

    # Mixup augmentation (ultra-aggressive for rapid convergence)
    mixup_p: float = 0.35
...
class VocalMorphDataset(Dataset):
    ...
    def _infer_source(self, data, speaker_id: str, file_path: str) -> str:
        ...
        if sid.startswith("NISP_"):
            return "NISP"
        if sid.startswith(("CELEB_", "HEIGHTCELEB_", "VOXCELEB_", "EXTERNAL_")):
            return "CELEB"
        name = os.path.basename(file_path).upper()
        if name.startswith("TIMIT_"):
            return "TIMIT"
        if name.startswith(("CELEB_", "HEIGHTCELEB_", "VOXCELEB_", "EXTERNAL_")):
            return "CELEB"
        return "NISP"
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # GPU-optimized tensor conversion
...
def collate_fn(batch: List[Dict]) -> Dict[str, torch.Tensor]:
    """Optimized collate function for GPU-accelerated training. Returns padding mask (True = padded)."""
    sequences = [item["sequence"] for item in batch]
    lengths = [s.shape[0] for s in sequences]
    max_len = max(lengths)

    # Optimized padding with pre-allocated tensor
    padded = pad_sequence(sequences, batch_first=True, padding_value=0.0)

    # Vectorized padding mask creation
...
def _sample_weight_from_npz(path: str, config: Dict[str, Any]) -> float:
    with np.load(path, allow_pickle=True) as data:
        height_raw = float(data["height_cm"]) if "height_cm" in data else float("nan")
        gender_raw = data["gender"] if "gender" in data else 0
        if isinstance(gender_raw, np.ndarray) and gender_raw.shape == ():
        ...
        source = "NISP"
        if "source" in data:
            source = VocalMorphDataset._decode_np_value(data["source"]).strip().upper()
        elif (
            "speaker_id" in data
            and VocalMorphDataset._decode_np_value(data["speaker_id"])
            .upper()
            .startswith("TIMIT_")
        ):
            source = "TIMIT"

    bin_name = height_bin(height_raw)
    gender_name = "male" if gender == 1 else "female"
    weight = 1.0
    weight *= float(config.get("height_bin_weights", {}).get(bin_name, 1.0))
    weight *= float(
        config.get("gender_height_weights", {}).get(f"{gender_name}_{bin_name}", 1.0)
    )
    weight *= float(
        config.get("source_height_weights", {}).get(f"{source.lower()}_{bin_name}", 1.0)
    )
    short_cm = float(config.get("extreme_short_cm", 152.0))
    tall_cm = float(config.get("extreme_tall_cm", 190.0))
    if np.isfinite(height_raw) and height_raw <= short_cm:
        weight *= float(config.get("extreme_short_weight", 1.0))
    if np.isfinite(height_raw) and height_raw >= tall_cm:
        weight *= float(config.get("extreme_tall_weight", 1.0))
    return float(np.clip(weight, 1e-3, float(config.get("max_weight", 25.0))))
...
def _build_weighted_sampler(
    ...
    *,
    ...
    if not bool(config.get("enabled", False)):
        return None
    weights = np.asarray(
        [_sample_weight_from_npz(path, config) for path in dataset.file_paths],
        dtype=np.float64,
    )
    if weights.size == 0 or not np.isfinite(weights).all():
        raise ValueError("Sample weighting produced non-finite weights")
    generator = torch.Generator()
    generator.manual_seed(int(base_seed) + 2029)
    print(
        "[VocalMorph Dataset] Weighted sampling: "
        f"min={weights.min():.2f} mean={weights.mean():.2f} max={weights.max():.2f}"
    )
...
def build_dataloaders_from_dirs(
    ...
    if train_batch_sampler is None:
        weighted_sampler = _build_weighted_sampler(
            train_ds, sample_weighting, base_seed=int(base_seed)
        )
