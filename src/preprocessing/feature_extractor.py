...
def extract_praat_features(
    ...
    # VTL = c / (2 * Δf)
    vtl_estimate = config.speed_of_sound / (2.0 * np.maximum(formant_spacing, 1.0))  # cm
...
def extract_all_features(
    ...
    vtl_mean = float(vtl_vals[vtl_vals > 0].mean()) if (vtl_vals > 0).any() else 0.0
    duration_s = float(len(audio) / max(config.sample_rate, 1))
    voiced_ratio = float(len(voiced) / max(T, 1))
    invalid_spacing_rate = float(np.mean(fs_vals <= 0.0)) if T > 0 else 1.0
    invalid_vtl_rate = float(np.mean(vtl_vals <= 0.0)) if T > 0 else 1.0
