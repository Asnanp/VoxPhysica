...
class BalancedHeightAwareSpeakerBatchSampler(Sampler[List[int]]):
    ...
    def _speaker_weight(
        ...
        height_bin = self.speaker_height_bin.get(speaker_id, "medium")
        ...
        if gender in (0, 1):
            ...
            weight *= self.gender_height_weights.get(
                f"{gender_name}_{height_bin}", 1.0
            )
        ...
        return float(min(self.max_speaker_weight, max(weight, 1e-6)))
