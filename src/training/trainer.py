...
def _build_summary_writer(log_dir: str):
    try:
        from torch.utils.tensorboard import SummaryWriter
    except (ImportError, AttributeError, RuntimeError, OSError) as exc:
        print(f"[Trainer] TensorBoard unavailable: {exc}")
        return None
...
class VocalMorphTrainer:
    def __init__(
        ...
        self.train_eval_frequency = max(0, int(train_cfg.get("train_eval_frequency", 1)))
        ...
        self.scheduler_steps_on_optimizer = self.scheduler_type.lower() in {
            "one_cycle_lr",
            "onecycle",
            "one_cycle",
        }
        ...
        # Aggressive training mode for 15-epoch convergence
        self.aggressive_mode = self.epochs <= 20
        if self.aggressive_mode:
            print("[Trainer] Aggressive 15-epoch training mode enabled")

        es_cfg = train_cfg.get("early_stopping", {})
        # Disable early stopping in aggressive 15-epoch mode
        self.early_stopping_enabled = es_cfg.get("enabled", True) and not self.aggressive_mode
        ...
        self.es_degradation_delta = float(es_cfg.get("degradation_delta", 0.0))
        self.es_degradation_patience = max(
        ...
        if tb_enabled:
            ...
            self.writer = _build_summary_writer(
                os.path.join(ROOT, tb_dir) if not os.path.isabs(tb_dir) else tb_dir
            )
        else:
            self.writer = None
        ...
        # For aggressive 15-epoch mode, start SWA earlier
        if self.aggressive_mode:
            swa_start_frac = max(0.5, swa_start_frac)  # Start at 50% for 15-epoch
        ...
        train_eval_status = "disabled"
        if self.train_eval_loader is not None and self.train_eval_frequency > 0:
            train_eval_status = f"every {self.train_eval_frequency} epoch(s)"
        print(f"[Trainer] Train-eval loader: {train_eval_status}")
    ...
    def _load_model_checkpoint_state(self, state_dict: Mapping[str, Any]) -> None:
        try:
            ...
            allowed_missing_prefixes = ("reliability_tower.", "height_bin_head.")
            ...
            if any(key.startswith("height_bin_head.") for key in missing_keys):
                alignment = getattr(self.model, "speaker_alignment_config", None)
                blend = float(getattr(alignment, "height_bin_prior_blend", 0.0) or 0.0)
                if alignment is not None and blend > 0.0:
                    self.model.speaker_alignment_config = replace(
                        alignment, height_bin_prior_blend=0.0
                    )
                    loss_module = getattr(self.model, "loss_module", None)
                    if getattr(loss_module, "speaker_alignment", None) is alignment:
                        loss_module.speaker_alignment = self.model.speaker_alignment_config
                    print(
                        "[Trainer] Disabled height-bin prior blend while loading a "
                        "checkpoint that does not contain height_bin_head weights."
                    )
            if missing_keys:
                print(
    ...
    def _enable_gradient_checkpointing(self):
        ...
        print("[Trainer] Gradient checkpointing enabled for memory efficiency")
    ...
    def _set_step_learning_rate(self, epoch: int, step_idx: int, n_steps: int) -> None:
        if self.scheduler_steps_on_optimizer:
            return
    ...
    @staticmethod
    def _add_height_guard_metrics(metrics: Dict[str, float]) -> None:
        for suffix in ("", "_omega"):
            overall_key = f"height_mae_speaker{suffix}"
            overall = metrics.get(overall_key)
            if not isinstance(overall, (int, float)) or not np.isfinite(overall):
                continue

            balanced_keys = [
                overall_key,
                f"height_heightbin_short_speaker_mae{suffix}",
                f"height_heightbin_medium_speaker_mae{suffix}",
                f"height_heightbin_tall_speaker_mae{suffix}",
            ]
            balanced_values = [
                float(metrics[key])
                for key in balanced_keys
                if isinstance(metrics.get(key), (int, float)) and np.isfinite(metrics[key])
            ]
            if balanced_values:
                metrics[f"height_mae_speaker_balanced{suffix}"] = float(
                    np.mean(np.asarray(balanced_values, dtype=np.float32))
                )

            guarded = float(overall)
            for key, weight in (
                (f"height_heightbin_short_speaker_mae{suffix}", 0.35),
                (f"height_quality_medium_speaker_mae{suffix}", 0.20),
                (f"height_heightbin_tall_speaker_mae{suffix}", 0.10),
            ):
                value = metrics.get(key)
                if isinstance(value, (int, float)) and np.isfinite(value):
                    guarded += float(weight) * max(0.0, float(value) - float(overall))
            metrics[f"height_mae_speaker_guarded{suffix}"] = guarded
    ...
    def _build_scheduler(
        ...
        if scheduler_type in {"one_cycle_lr", "onecycle", "one_cycle"}:
            try:
                from torch.optim.lr_scheduler import OneCycleLR
                
                max_lr = float(sched_cfg.get("max_lr", 0.001))
                pct_start = float(sched_cfg.get("pct_start", 0.3))
                anneal_strategy = str(sched_cfg.get("anneal_strategy", "cos")).strip().lower()
                ...
                explicit_total_steps = sched_cfg.get("total_optimizer_steps")
                legacy_total_steps = sched_cfg.get("total_steps")
                if explicit_total_steps is None and legacy_total_steps is not None:
                    legacy_total_steps_int = int(legacy_total_steps)
                    if legacy_total_steps_int > self.epochs:
                        explicit_total_steps = legacy_total_steps_int
                    else:
                        print(
                            "[FIX:3cm] OneCycleLR total_steps looked epoch-based; "
                            f"using {self.epochs * optimizer_steps_per_epoch} optimizer steps instead."
                        )
                ...
                if explicit_total_steps is not None:
                    one_cycle_kwargs["total_steps"] = int(explicit_total_steps)
                else:
                    one_cycle_kwargs["epochs"] = self.epochs
                    one_cycle_kwargs["steps_per_epoch"] = optimizer_steps_per_epoch
                print(
                    "[FIX:3cm] OneCycleLR stepping per optimizer update "
                    f"({optimizer_steps_per_epoch} steps/epoch)."
                )
                return OneCycleLR(self.optimizer, **one_cycle_kwargs)
            except ImportError:
                print("[Trainer] OneCycleLR not available, falling back to cosine annealing")
                # Fallback to cosine annealing
                t_max = max(1, int(sched_cfg.get("total_steps", self.epochs)))
    ...
    def _val_epoch_on(self, loader, split_name: str = "eval"):
        ...
        try:
            ...
            self._add_height_guard_metrics(metrics)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            should_train_eval = (
                ...
                and self.train_eval_frequency > 0
                and (
                    epoch == self.start_epoch
                    or epoch % self.train_eval_frequency == 0
                )
            )
            if should_train_eval:
            ...
            # Step scheduler for non-OneCycleLR schedulers (per epoch)
            if not self.scheduler_steps_on_optimizer:
                self.scheduler.step()
            
            # GPU memory management - periodic cache clearing
            if self.empty_cache_frequency > 0 and epoch % self.empty_cache_frequency == 0:
                if self.device.type == "cuda":
                    torch.cuda.empty_cache()
                    print(f"[Trainer] GPU cache cleared at epoch {epoch}")
            ...
            speaker_height_balanced = val_metrics.get(
                "height_mae_speaker_balanced", float("nan")
            )
            gap = float("nan")
            if train_eval_metrics:
            ...
            if improved:
                ...
                degraded = False
                if (
                    self.es_degradation_patience > 0
                    and self.es_degradation_delta > 0.0
                    and np.isfinite(monitored)
                    and np.isfinite(self.best_val_metric)
                ):
                    if self.es_mode == "min":
                        degraded = monitored > (
                            self.best_val_metric + self.es_degradation_delta
                        )
                    else:
                        degraded = monitored < (
                            self.best_val_metric - self.es_degradation_delta
            ...
            print(
                ...
                f"h_bal={speaker_height_balanced:.2f}cm "
            ...
            print(
                f"              bins short={val_metrics.get('height_heightbin_short_speaker_mae', float('nan')):.2f}cm "
                f"medium={val_metrics.get('height_heightbin_medium_speaker_mae', float('nan')):.2f}cm "
                f"tall={val_metrics.get('height_heightbin_tall_speaker_mae', float('nan')):.2f}cm"
            )
            if train_eval_metrics:
                print(
            ...
            if (
                ...
                print(
                    "\n[Early Stop] "
                    f"{self.es_monitor} degraded by more than "
                    f"{self.es_degradation_delta:.4f} for "
                    f"{self.es_degradation_patience} epochs."
                )
                break
        ...
        if self.use_swa and self.swa.enabled and self.swa_use_for_eval:
            ...
            if swa_backup:
                ...
                if (self.es_mode == "min" and swa_monitor < self.best_val_metric) or (
                    ...
                    swa_checkpoint_state = self._build_checkpoint_state(
                        epoch=self.last_completed_epoch,
                        metric_val=float(swa_monitor),
                        train_eval_metrics=self.last_train_eval_metrics,
                        val_metrics=swa_val,
                        termination_reason="swa_best",
                    )
                    self.ckpt_manager.save(
                        swa_checkpoint_state, float(swa_monitor), is_best=True
                    )
                    print("  [FIX:3cm] Saved SWA weights as best checkpoint.")
                else:
                    print(
