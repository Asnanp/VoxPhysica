...
from src.training.trainer import ROOT as TRAINER_ROOT, VocalMorphTrainer
...
def test_trainer_root_points_to_project_root():
    assert os.path.basename(TRAINER_ROOT) != "src"
    assert os.path.isdir(os.path.join(TRAINER_ROOT, "src"))
    assert os.path.isdir(os.path.join(TRAINER_ROOT, "configs"))
...
def test_v2_trainer_uses_native_loss_and_reports_speaker_metrics(tmp_path):
    ...
    assert "height_mae_speaker_balanced" in val_metrics
    assert "height_mae_speaker_guarded" in val_metrics
    ...
    assert torch.isfinite(torch.tensor(val_metrics["height_mae_speaker_balanced"]))
    assert torch.isfinite(torch.tensor(val_metrics["height_mae_speaker_guarded"]))
...
def test_trainer_steps_one_cycle_per_optimizer_update(tmp_path):
    ...
    assert trainer.scheduler_steps_on_optimizer is True
    assert trainer.scheduler.total_steps == 4
    losses = trainer._train_epoch(epoch=1)
    assert torch.isfinite(torch.tensor(losses["total"]))
