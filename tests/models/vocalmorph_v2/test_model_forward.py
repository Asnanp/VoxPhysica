...
def test_forward_shapes_include_optional_targets(small_model, batch_size, time_steps):
    ...
    assert out["height_bin_logits"].shape == (batch_size, 3)
    assert out["height_bin_probs"].shape == (batch_size, 3)
    assert torch.allclose(out["height_bin_probs"].sum(dim=-1), torch.ones(batch_size), atol=1e-5)
...
def test_builder_maps_training_mixup_probability_to_v2_model():
    config = {
        "model": {
            "input_dim": 136,
            "v2": {
        ...
        "training": {"augmentation": {"mixup_p": 0.0, "mixup_alpha": 0.4}},
    }
    model = build_vocalmorph_v2(config)
    assert model.mixup_probability == 0.0
    assert model.mixup_alpha == 0.4

    model.train()
    features, mask, domain, targets = make_batch(batch_size=4, time_steps=8)
    out = model(features, padding_mask=mask, domain=domain, targets=targets)
    assert not bool(out["mixup"]["applied"].any())
